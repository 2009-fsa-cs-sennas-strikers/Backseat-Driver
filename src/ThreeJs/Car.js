...
const Car = (props) => {
  ...
  let rotationalPivot = 0;

  useFrame(() => {
    if (rotationalPivot === (360 || -360)){
      rotationalPivot = 0
    }
    api.velocity.set(0,0,0)
    if (props.action === 'right') {
      rotationalPivot = rotationalPivot
     // api.applyForce(25, [500,0,0])
      api.rotation.set(0, (Math.PI * rotationalPivot/180), 0)
    }
